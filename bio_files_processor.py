            ...
            for genline in gene_list:
                if lines[genline].find(word) != -1:
                    for i in range(n_before):
                        list_res.append(
                            ''.join(lines[trans_list[gene_list.index(genline)] - n_before + 1:genline - n_before]))
                    for i in range(n_after):
                        list_res.append(''.join(lines[trans_list[gene_list.index(genline) + n_after + 1]:gene_list[
                                                                                                             gene_list.index(
                                                                                                                 genline) + n_after + 1] - 1]))
